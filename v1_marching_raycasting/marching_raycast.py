...
class Window:
    ...
    def find_square(self, x, y):
        ...
        x_square = int(np.floor( (x-border_x) / grid_x ))
        grid_y = self.maze_y / self.matrix_y         
        y_square = int(np.floor( (y-border_y) / grid_y ))
    ...
    def raycasting(self, angle):
        ...
        while line_length < self.max_length:
            ...
            x_square, y_square = self.find_square(x2, y2)
    ...
    def calculate_distant_to_wall(self):
        ...
        for theta in range( int(-self.field_view_angle/2), int(self.field_view_angle/2), step):
            ...
            line_length = self.raycasting(angle)
    ...
    def can_move(self, x, y):
        ...
        for ox, oy in offsets:
            x_sq, y_sq = self.find_square(x + ox, y + oy)
    ...
    def three_d_view(self):
        ...
        for length, angle in self.length_list:
            if length < self.max_length:
                ...
                # colour = 255 / (1 + corrected_length * 0.15)
                # colour = max(40, min(200, int(colour)))

                #Old fish eye method
                corrected_length = length
                height = (1 / corrected_length) * self.maze_y * 10 +20
                if height > 450:
                    height = 450
                colour = (1/corrected_length) * 5000 + 75
                if colour > 200:
                    colour = 200
    ...
    def main_loop(self, vel):
        run = True
        while run:
            pygame.time.delay(1) # Time Delay
